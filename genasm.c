/* genasm.c       Generate Assembly Code for X86    ; 10 Aug 12   */
...
/* Example:  asmimmed(MOVL, i, reg);   Moves i to reg  */
...
/* Example:  asmldr(MOVL, 4, EAX, ECX, code->stringval);  4(%rax) --> %ecx */
...
/* Example:
   asmldrr(MOVL, -8, EAX, ECX, code->stringval);  -8(%rbp,%rax) --> %ecx */
...
/* Example:
   asmldrrm(MOVL, -8, EAX, ECX, 4, code->stringval); -8(%rbp,%rax,4) --> %ecx */
...
/* Generate a store instruction relative to RBP and another register: */
/* Example:  asmstr(MOVL, ECX, 4, EAX, code->stringval);  %ecx --> 4(%rax) */
...
/* Example:
   asmstrr(MOVL, ECX, -8, EAX, code->stringval);  -8(%rbp,%rax) --> %ecx */
...
/* Generate a load instruction using offset, RBP and
   another register with multiplier (2, 4 or 8): */
/* Example:
   asmstrrm(MOVL, ECX, -8, EAX, 4, code->stringval); -8(%rbp,%rax,4) --> %ecx */
...
/* Linux note: you may need to switch the [0] and [1] below. */
int lefth(d)
  double d;
  {  dtoitmp.val.dbl = d;
